...
const marFeedFetched = `-- name: MarFeedFetched :exec
UPDATE feeds
SET last_fetched_at = $1, updated_at = $2
WHERE id = $3
`

type MarFeedFetchedParams struct {
	LastFetchedAt sql.NullTime
	UpdatedAt     time.Time
	ID            uuid.UUID
}

func (q *Queries) MarFeedFetched(ctx context.Context, arg MarFeedFetchedParams) error {
	_, err := q.db.ExecContext(ctx, marFeedFetched, arg.LastFetchedAt, arg.UpdatedAt, arg.ID)
	return err
}
